...
void pointRotate(point o, point in, double ang ,point &out) {
    ...
    out.x = ((in.x - o.x) * cos(ang)) - ((o.y - in.y) * sin(ang)) + o.x;
    out.y = ((o.y - in.y) * cos(ang)) - ((in.x - o.x) * sin(ang)) + o.y;
    if( fabs(out.x - 0.0) < EPS )
        out.x = 0.0;
    if( fabs(out.y - 0.0) < EPS )
        out.y = 0.0;
...
int main() {
    ...
    while(cin >> n && n) {
        ...
        //cerr << setprecision(3) << fixed << R << endl;
        vector <point> p (6);
        circle2PtsRad(a,b,R,p[0]);
        circle2PtsRad(b,a,R,p[1]);
        circle2PtsRad(a,c,R,p[2]);
        circle2PtsRad(c,a,R,p[3]);
        circle2PtsRad(b,c,R,p[4]);
        circle2PtsRad(c,b,R,p[5]);
        point o;
        ...
        double minX = tmp.x , minY = tmp.y , maxX = tmp.x , maxY = tmp.y;
        double ang = (double)(2.0)* PI / (double)(n);
        //cerr << ang << endl;
        for (int i = 0; i < n; i++) {

            pointRotate(o,tmp,ang*i,tmp2);

            //cerr << setprecision(3) << fixed << tmp2.x << ' ' << tmp2.y << endl;
            //cerr << ' ' << maxX << ' ' << maxY << endl;
            minX = min(minX,tmp2.x);
            minY = min(minY,tmp2.y);
            maxX = max(maxX,tmp2.x);
            maxY = max(maxY,tmp2.y);

        }
        //cerr << maxX << ' ' << maxY << endl;
        double totalX = fabs( maxX - minX);
        double totalY = fabs( maxY - minY);

        double area = totalX * totalY;
        cout << "Polygon " << ++k << ": ";
